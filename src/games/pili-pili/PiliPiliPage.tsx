...
import PiliPiliForeheadSelect from './components/PiliPiliForeheadSelect';
import PiliPiliTransferSelect from './components/PiliPiliTransferSelect';
...
export default function PiliPiliPage() {
  ...
  const {
    ...
    nextRound, newGame,
    foreheadSelect, piliTransferSelect,
  ...
  const handleCreateRoom = async (pName: string) => {
    setIsConnecting(true); await createRoom(pName); setIsConnecting(false); setLocalPhase('lobby');
  };
  const handleJoinRoom = async (code: string, pName: string): Promise<boolean> => {
    setIsConnecting(true); const ok = await joinRoom(code, pName); setIsConnecting(false);
